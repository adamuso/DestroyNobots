...
namespace DestroyNobots.Assembler.Parser
{
    public class AssemblerCompiler
    {
        ...
        public void SetInstruction(string name, byte opcode, InstructionAction action, int params_number, bool label_offset, params AssemblerParameters[] parameters)
        {
            ...
            AssemblerInstruction instructon = new AssemblerInstruction(this, action, label_offset);
            computer.RegisterInstruction(opcode, instructon.SetParameters(parameters));
        }
        ...
        private void PreProcess(System.IO.Stream stream)
        {
            ...
            while (reader.BaseStream.Length != reader.BaseStream.Position)
            {
                ...
                if (opcodes.Count == 1)
                    parnum = computer.Instructions[opcodes.Opcodes[0]].ParametersCount;
                ...
                for (int i = 0; i < parnum; i++)
                {
                    ...
                    if (char.IsLetter((char)reader.PeekChar()) || (char)reader.PeekChar() == '_')
                    {
                        ...
                        else if (labelNames.Contains(reg))
                        {
                            ...
                            _params.Add(AssemblerParameters.POINTER);
                        }
                        else
                        {
                            args.Add(registers[reg.ToLower()]);
                            _params.Add(pointer ? AssemblerParameters.POINTER | AssemblerParameters.REGISTER : AssemblerParameters.REGISTER);
                        }
                    }
                ...
                //ASMInstruction asm = computer.Instructions[opcode];
                byte paramstypes = 0;
                ...
                for (int i = 0; i < args.Count; i++)
                {
                    ...
                    else if (_params[i] == AssemblerParameters.VALUE)
                    {
        ...
        public void Compile(System.IO.Stream stream)
        {
            ...
            while (reader.BaseStream.Length != reader.BaseStream.Position)
            {
                ...
                byte opcode = opcodes.Find(_params.ToArray());
                Parser.AssemblerInstruction asm = computer.Instructions[opcode];
                byte paramstypes = 0;
            ...
            computer.Memory.Write(computer.ProgramMemory, buffer.ToArray());
        }
